...
> [!TIP]
>
> Location within the framework `Isaline-agent-framework/cache`.
...
```ts
import { UnconstrainedCache } from "Isaline-agent-framework/cache/unconstrainedCache";
...
```ts
import { SlidingCache } from "Isaline-agent-framework/cache/slidingCache";
import { WikipediaTool } from "Isaline-agent-framework/tools/search/wikipedia";
...
import { BaseMessage } from "Isaline-agent-framework/llms/primitives/message";
...
```ts
import { FileCache } from "Isaline-agent-framework/cache/fileCache";
import * as os from "node:os";

const cache = new FileCache({
  fullPath: `${os.tmpdir()}/Isaline_file_cache_${Date.now()}.json`,
});
...
const fileCache = await FileCache.fromProvider(memoryCache, {
  fullPath: `${os.tmpdir()}/Isaline_file_cache.json`,
});
...
```ts
import { Cache } from "Isaline-agent-framework/cache/decoratorCache";
...
```ts
import { Cache, SingletonCacheKeyFn } from "Isaline-agent-framework/cache/decoratorCache";
...
```ts
import { CacheFn } from "Isaline-agent-framework/cache/decoratorCache";
...
```ts
import { BaseCache } from "Isaline-agent-framework/cache/base";
import { NotImplementedError } from "Isaline-agent-framework/errors";
