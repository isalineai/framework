...
## Implementation in Isaline Agent Framework
...
Our Isaline Agent is based on the `ReAct` ([Reason and Act](https://arxiv.org/abs/2210.03629)) approach.
...
const agent = new IsalineAgent({
  llm: new OllamaChatLLM(), // for more explore 'Isaline-agent-framework/adapters'
  memory: new UnconstrainedMemory(), // for more explore 'Isaline-agent-framework/memory'
  tools: [new OpenMeteoTool()], // for more explore 'Isaline-agent-framework/tools'
});
...
Please refer to the [following example](/examples/agents/Isaline_advanced.ts) to see how to modify them.
