...
> [!TIP]
>
> Location within the framework `Isaline-agent-framework/logger`.
...
In the Isaline Agent Framework, the [Logger](/src/logger/logger.ts) class is an abstraction built on top of the popular [pino](https://github.com/pinojs/pino) logger, offering additional flexibility and integration.
...
```ts
import { Logger, LoggerLevel } from "Isaline-agent-framework/logger/logger";

// Configure logger defaults
Logger.defaults.pretty = true; // Pretty-print logs (default: false, can also be set via ENV: Isaline_FRAMEWORK_LOG_PRETTY=true)
Logger.defaults.level = LoggerLevel.TRACE; // Set log level to trace (default: TRACE, can also be set via ENV: Isaline_FRAMEWORK_LOG_LEVEL=trace)
...
import { UnconstrainedMemory } from "Isaline-agent-framework/memory/unconstrainedMemory";
import { Logger } from "Isaline-agent-framework/logger/logger";
import { Emitter } from "Isaline-agent-framework/emitter/emitter";
...
If you need to integrate your own `pino` instance with the Isaline Agent Framework Logger, you can do so easily. Below is an example that demonstrates how to create a pino logger and use it with the framework’s [Logger](/src/logger/logger.ts).
