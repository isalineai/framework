...
> Location within the framework `Isaline-agent-framework/emitter`.
...
```ts
import { Emitter, EventMeta } from "Isaline-agent-framework/emitter/emitter";
...
```ts
import { Emitter, EventMeta, Callback } from "Isaline-agent-framework/emitter/emitter";
...
await emitter.emit("update", { id: 123, data: "Hello Isaline!" });
```
...
```ts
import { Callback, Emitter } from "Isaline-agent-framework/emitter/emitter";
import { BaseLLM } from "Isaline-agent-framework/llms/base";
...
> [!TIP]
>
> The more complex agentic example can be found [here](/examples/agents/Isaline.ts).
